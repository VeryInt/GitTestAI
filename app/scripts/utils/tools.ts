...
export const getGraphqlAIMashupBody = ({ prompt, name, isStream, queryQwen, qwenParams, queryGeminiPro, geminiProParams, queryMoonshot, moonshotParams, queryGroq, groqParams, queryClaude, claudeParams, queryErnie, ernieParams, queryOpenAI, openAIParams, queryLingyiwanwu, lingyiwanwuParams }: IGrahpqlAIFetchProps & { name?: string})=>{
    ...
    if(queryGeminiPro){
        let paramsGeminiPro = '', hasGeminiProArgs = geminiProParams;
        if(hasGeminiProArgs){
    ...
    if(queryMoonshot){
        let paramsMoonshot = '', hasMoonshotArgs = moonshotParams;
        if(hasMoonshotArgs){
    ...
    if(queryGroq){
        let paramsGroq = '', hasGroqArgs = groqParams;
        if(hasGroqArgs){
    ...
    if(queryClaude){
        let paramsClaude = '', hasClaudeArgs = claudeParams;
        if(hasClaudeArgs){
    ...
    if(queryErnie){
        let paramsErnie = '', hasErnieArgs = ernieParams;
        if(hasErnieArgs){
    ...
    if(queryOpenAI){
        let paramsOpenAI = '', hasOpenAIArgs = openAIParams;
        if(hasOpenAIArgs){
    ...
    if(queryLingyiwanwu){
        let paramsLingyiwanwu = '', hasLingyiwanwuArgs = lingyiwanwuParams;
        ...
        queryList.push(isStream ? `LingyiwanwuStream${paramsLingyiwanwu}@stream` : `Lingyiwanwu ${paramsLingyiwanwu} {text}`)
    }
